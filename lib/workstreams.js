 ...
 * Each color has a hex (UI swatch) and an ANSI 256-color code (statusline).
 * Stored on the workstream as a name; resolved here for both UI and shell consumers.
 */
const WORKSTREAM_COLORS = {
  red:    { hex: '#ef4444', ansi: '\\033[38;5;203m' },
  orange: { hex: '#f97316', ansi: '\\033[38;5;208m' },
  yellow: { hex: '#eab308', ansi: '\\033[38;5;221m' },
  green:  { hex: '#22c55e', ansi: '\\033[38;5;120m' },
  cyan:   { hex: '#06b6d4', ansi: '\\033[38;5;87m'  },
  blue:   { hex: '#3b82f6', ansi: '\\033[38;5;111m' },
  purple: { hex: '#a855f7', ansi: '\\033[38;5;177m' },
  pink:   { hex: '#ec4899', ansi: '\\033[38;5;213m' },
  gray:   { hex: '#9ca3af', ansi: '\\033[38;5;245m' },
};

function isValidColor(name) {
  return typeof name === 'string' && Object.prototype.hasOwnProperty.call(WORKSTREAM_COLORS, name);
}
 ...
 */
function workstreamCreate(installDir, name, projects = [], rules = '', color = null) {
  if (!name) {
  ...
  if (isValidColor(color)) workstream.color = color;
...
function workstreamUpdate(installDir, idOrName, updates) {
  ...
  if (updates.color !== undefined) {
    if (updates.color === null || updates.color === '') {
      delete ws.color;
    } else if (isValidColor(updates.color)) {
      ws.color = updates.color;
    }
  }
...
function workstreamUse(installDir, idOrName, evalMode = false) {
  ...
  if (evalMode) {
    ...
    if (isValidColor(ws.color)) {
      console.log(`export CODER_WORKSTREAM_COLOR="${ws.color}"`);
    } else {
      console.log(`unset CODER_WORKSTREAM_COLOR`);
    }
...
/**
 * Set color for a workstream.
 * Accepts a color name from WORKSTREAM_COLORS, or 'none'/'' to clear.
 */
function workstreamSetColor(installDir, idOrName, value) {
  const data = loadWorkstreams(installDir);
  const ws = data.workstreams.find(
    w => w.id === idOrName || w.name.toLowerCase() === idOrName.toLowerCase()
  );
  ...
  if (value === 'none' || value === '' || value === null) {
    delete ws.color;
    console.log(`✓ Cleared color for ${ws.name}`);
  } else if (isValidColor(value)) {
    ws.color = value;
    console.log(`✓ Set color for ${ws.name}: ${value}`);
  } else {
    const valid = Object.keys(WORKSTREAM_COLORS).join(', ');
    console.error(`Invalid color: ${value}. Valid: ${valid}, none`);
    return null;
  }
...
_coder_workstream_cd() {
  ...
  if [ "$count" = "1" ]; then
    ...
    color=$(echo "$result" | grep -o '"color":"[^"]*"' | head -1 | cut -d'"' -f4)
    export CODER_WORKSTREAM="$id"
    if [ -n "$color" ]; then export CODER_WORKSTREAM_COLOR="$color"; else unset CODER_WORKSTREAM_COLOR; fi
    ...
    if [ "$choice" -gt 0 ] 2>/dev/null; then
      ...
      color=$(echo "$result" | grep -o '"color":"[^"]*"' | sed -n "\${choice}p" | cut -d'"' -f4)
      if [ -n "$id" ]; then
        export CODER_WORKSTREAM="$id"
        if [ -n "$color" ]; then export CODER_WORKSTREAM_COLOR="$color"; else unset CODER_WORKSTREAM_COLOR; fi
        echo "📂 Workstream: $name"
