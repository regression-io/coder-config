export const permissionsContent = {
  'understanding-permissions': {
    ...
    content: `
Permissions control how much autonomy your AI has. Without them, your AI either asks permission for everything (slow and tedious) or runs in full trust mode (fast but risky). Permissions let you find the middle ground—automatic approval for safe operations, manual review for sensitive ones.

### The Three Levels

**Allow** means your AI proceeds without asking. Use this for operations that are safe and frequent. Reading source code files, running test suites, checking git status—things you'd approve 100% of the time anyway. Allowing them saves you from constant "May I?" dialogs.

**Ask** means your AI pauses and requests permission. Use this for operations you want to review before they happen. Installing npm packages, editing configuration files, running database migrations—things that could cause problems if done wrong. You see what your AI wants to do and decide whether to approve.

**Deny** means your AI cannot perform that operation at all. Use this for things that should never happen automatically. Deleting production files, pushing to main without review, exposing secrets—operations where the risk isn't worth the convenience.

### Finding the Balance

The goal is smooth flow for routine work while keeping guardrails on risky operations. Start by thinking about your typical development session. What does your AI do repeatedly that you always approve? Those operations should be allowed. What operations would you want to review before they happen? Those should be ask. What should never happen? Those should be denied.
...
Good permissions make your AI feel like a trusted collaborator rather than an overeager intern who needs constant supervision. You can give your AI a task and let it work without interrupting you every few seconds. But you still have oversight where it counts.

Bad permissions—either too restrictive or too loose—create friction. Too restrictive and you're just clicking "approve" constantly. Too loose and you're anxious about what your AI might do while you're not looking.
    `
  },
  'setting-permissions': {
...
If you're not sure where to begin, here's a sensible configuration. In the **Allow** column, add \`Read(**)\` so your AI can read any file—it needs to understand your code to help with it. Add \`Bash(npm:test)\` to let it run tests freely, and \`Bash(git:status)\` for checking git state. These are all read-only or safe operations.
...
After configuring, try working with your AI normally. If you find yourself approving the same thing repeatedly, consider allowing it. If something happens that makes you uncomfortable, add an ask or deny rule. Permissions are meant to be tuned over time as you learn what works for your workflow.
    `
  },
  'permission-patterns': {
...
File patterns control Read, Edit, and Write operations. The pattern \`**\` matches everything at any depth, so \`Read(**)\` means your AI can read any file anywhere in your project. The pattern \`*.js\` matches JavaScript files but only in the current directory—if you want all JavaScript files at any depth, use \`**/*.js\` instead. The pattern \`src/**\` matches everything under the src folder. For an exact file match, just use the filename: \`.env\` matches exactly that file and nothing else. You can also use brace expansion: \`*.{ts,tsx}\` matches both TypeScript and TSX files.

### Bash Patterns

Bash patterns control what shell commands your AI can run. The format is \`command:arguments\` where the colon separates the base command from its arguments.
...
MCP patterns control which tools from which MCP servers your AI can use. The format uses double underscores: \`mcp__servername__toolname\`.
